  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      <button
        ...
        className="px-2 py-1.5 border border-sand-200 rounded-lg text-sm hover:bg-sand-50 transition-colors"
      ...
      {showMonth && (
        <select
          value={currentMonth}
          onChange={(e) => onChange({ month: parseInt(e.target.value), year: currentYear })}
      ...
      {showYear && (
        <input
          type="number"
          value={currentYear}
          onChange={(e) => onChange({ month: currentMonth, year: parseInt(e.target.value) || currentYear })}
