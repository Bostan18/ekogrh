...
export default function OnboardingChecklist({ onDismiss }) {
  ...
  if (steps.length > 0 && steps.filter((s) => !s.loading).every((s) => s.done)) {
    localStorage.setItem('ekogrh_onboarding_dismissed', 'true')
    return null
  }

  return (
    <div className="bg-white rounded-xl shadow-card border border-gold-200 p-6 mb-6">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between text-xs text-sand-500 mb-1.5">
          ...
          <span>{doneCount}/{totalCount} étapes</span>
        </div>
        ...
        {steps.map((step) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium ${step.done ? 'text-green-700' : 'text-ink'}`}>
                {step.label}
              </p>
            ...
            {!step.done && !step.loading && (
              <Link
                to={step.link}
                className="flex-shrink-0 px-3 py-1 text-xs font-medium rounded-lg bg-forest-500 hover:bg-forest-600 text-white transition-colors"
              >
                Commencer
              </Link>
            )}
