  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-display font-bold text-ink">Employés</h2>
        <Link
          to="/employes/nouveau"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-sand-100 bg-sand-50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-sand-500 uppercase">Code</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-sand-500 uppercase">Nom</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-sand-500 uppercase">Taux</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-sand-50">
              {employes.map((emp) => (
                <tr key={emp.id} className="hover:bg-sand-50 transition-colors">
                  <td className="px-4 py-3">
                    <Link to={} className="text-forest-600 font-mono text-sm hover:underline">
                      {emp.code}
                    </Link>
                  </td>
                  <td className="px-4 py-3">
                    <Link to={} className="text-ink font-medium hover:text-forest-600">
                      {emp.nom_complet}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-sm text-sand-600">{emp.poste || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={}>
                  ...
                  <td className="px-4 py-3 text-sm text-sand-700">
                    {emp.salaire_mensuel ?  : emp.taux_journalier ?  : '—'}
                  </td>
                </tr>
              ))}
              {employes.length === 0 && (
