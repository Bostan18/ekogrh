...
export default function PointageSemaine() {
  ...
  useEffect(() => { load(semaine) }, [semaine, load])
  ...
  const togglePresence = (ligneIdx, jourIdx) => {
    ...
    ligne.jours.forEach(j => {
      if (j.present) total += parseFloat(j.montant_du || ligne.taux_journalier || 0)
    })
    ligne.total_montant = String(total)

    setData({ ...data, lignes })
    setModified(prev => new Set(prev).add(`${ligne.employe_id}-${jour.date}`))
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/rh/presences/saisie_semaine/', { lignes: payload })
      setModified(new Set())
      load(semaine)
    } catch (err) {
  ...
  const cellClass = (present) => {
    if (present === true) return 'bg-green-100 text-green-700 hover:bg-green-200'
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <h2 className="text-2xl font-display font-bold text-ink">Pointage — Semaine</h2>
        <div className="flex items-center gap-2">
          <button onClick={prevWeek} className="px-3 py-1.5 border border-sand-200 rounded-lg text-sm hover:bg-sand-50 transition-colors">
            ← Précédent
          </button>
          <span className="text-sm font-medium text-sand-700 min-w-[180px] text-center">
            {data?.semaine_debut ? formatDate(data.semaine_debut) : ''} →{' '}
            {data?.jours?.length ? formatShort(data.jours[6]) : ''}
          </span>
          <button onClick={nextWeek} className="px-3 py-1.5 border border-sand-200 rounded-lg text-sm hover:bg-sand-50 transition-colors">
            Suivant →
          </button>
        </div>
      </div>
      ...
      {data && (
        <div className="flex items-center gap-4 mb-4 text-xs text-sand-500">
          <span>
            <span className="inline-block w-4 h-4 bg-green-100 rounded mr-1 align-middle"></span> Présent
          </span>
          <span>
            <span className="inline-block w-4 h-4 bg-red-50 rounded mr-1 align-middle"></span> Absent
          </span>
          <span>
            <span className="inline-block w-4 h-4 bg-sand-50 rounded mr-1 align-middle"></span> Non pointé
          </span>
          <span className="ml-auto">
            {data.lignes.length} journaliers · Modifications: {modified.size}
          </span>
        </div>
      )}
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-sand-100 bg-sand-50">
                ...
                {data.jours.map((jour) => (
                  <th key={jour} className="text-center px-2 py-2 text-xs font-semibold text-sand-500 uppercase w-[9%]">
                    {formatShort(jour)}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-sand-50">
              {data.lignes.map((ligne, li) => (
                <tr key={ligne.employe_id} className="hover:bg-sand-50/50 transition-colors">
                  <td className="px-3 py-2 sticky left-0 bg-white z-10">
                    <div className="text-sm font-medium text-ink truncate max-w-[160px]">
                      {ligne.employe_nom}
                    </div>
                    <div className="text-xs text-sand-500">{ligne.employe_code}</div>
                  </td>
                  {ligne.jours.map((jour, ji) => {
                    const modKey = `${ligne.employe_id}-${jour.date}`
                    const isModified = modified.has(modKey)
                    return (
                      <td key={jour.date} className="text-center px-1 py-2">
                        <button
                          onClick={() => togglePresence(li, ji)}
                  ...
                  <td className="text-center px-3 py-2">
                    <span className="text-sm font-semibold text-ink">
                      {parseFloat(ligne.total_montant || 0).toLocaleString()} F
                    </span>
                  </td>
                </tr>
              ))}
      ...
      <div className="mt-4 flex justify-end">
        ...
        >
          {saving ? 'Enregistrement...' : modified.size > 0 ? `Enregistrer (${modified.size})` : 'Enregistrer'}
        </button>
      </div>
    </div>
  )
